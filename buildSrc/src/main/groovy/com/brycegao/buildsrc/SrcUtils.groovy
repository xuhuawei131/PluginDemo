...
class SrcUtils {
   ...
   * @param project
   */
  static void injectTimeCode(String path, Project project, SrcConfig config) {
    println("SrcUtils add path:" + path)
    sPool.appendClassPath(path)
    project.android.bootClasspath.each {
      sPool.appendClassPath(it.absolutePath)
      println("SrcUtils appendBootClassPath:" + it.absolutePath)  //android.jar的路径
    }
        ...
        if (filePath.endsWith(".class") && !filePath.contains('R$') &&
          ...
          if (ctClass.getAnnotations() != null) {
            for (int i=0; i<ctClass.getAnnotations().size(); i++) {
              if (ctClass.getAnnotations()[i].toString().contains("CostTime")) {
                classAllInject = true
                println("类注解")
              }
            ...
            if (classAllInject) {
              ...
              if (objects != null && objects.length > 0) {
                for (java.lang.Object object : objects) {
                  if (object.toString().contains("CostTime")) {
                    methodInject = true
                    break
                  }
                }
  ...
  static byte[] visitAndReturnCode(byte[] bytes) {
    ...
    MethodInjectAdatper classAdapter = new MethodInjectAdatper(classWriter)
    try {
